...
pub fn cmd_replay(
    ...
    println!("\n{} Replaying spec {}", "→".cyan(), spec_id.cyan());
    println!("  Original date: {}", completion_date);
    println!("  Current date: {}", current_date);

    if let Some(ref prompt_arg) = prompt {
        println!("  Using prompt: {}", prompt_arg);
    }

    if let Some(ref branch_arg) = branch {
        println!("  Target branch: {}", branch_arg);
    }

    if dry_run {
        println!("\n{} Dry run mode - no changes will be made", "ℹ".blue());
        return Ok(());
    }

    // Confirm with user unless --yes or --force is set
    if !yes && !force {
        let confirmed = chant::prompt::confirm("Proceed with replay?")?;
        if !confirmed {
    ...
    // Create a replay context for future use
    let _original_completion = spec
        .frontmatter
        .completed_at
        .clone()
        .unwrap_or_else(|| chrono::Utc::now().format("%Y-%m-%dT%H:%M:%SZ").to_string());

    let _replay_context = chant::replay::ReplayContext::new(
        spec_id.clone(),
        spec.title.clone(),
        _original_completion,
        None, // replay_reason defaults to "manual"
    );

    println!("\n{} Executing replay...", "→".cyan());

    // For now, just call cmd_work with the spec
    // The replay context could be used for commit message formatting
    // This is a placeholder - full replay implementation would need more work
    println!("{} Replay functionality is under development", "⚠".yellow());
    println!("  You can manually re-execute with: chant work {}", spec_id);

    // Uncommenting this would require work module to accept replay context:
    // cmd::work::cmd_work(

...
- **Circular dependency blocking in member specs**: `is_blocked()` made member specs inherit driver's `depends_on`, causing members to block themselves when the driver depends on its own members (as generated by `chant split`). Members now skip sibling/self references in driver dependencies.
- **Sequential sibling check blocks on cancelled/missing siblings**: `all_prior_siblings_completed()` treated cancelled and missing siblings as incomplete, permanently blocking later members. Cancelled siblings are now treated like completed, and missing siblings are skipped.
- **Orchestrator instructions leaking to worker agents**: MCP tool responses appended "do not edit files directly" warning that worker agents also received, causing them to refuse file edits. Removed the MCP-level warning and added explicit file-editing permission to worker bootstrap prompts.

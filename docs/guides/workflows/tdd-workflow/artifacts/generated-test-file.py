...
This file demonstrates the test output generated by a chant agent
when executing a test planning spec.
"""
...
class TestRefundEdgeCases:
    ...
    def test_refund_during_processor_outage_queued(
        self, mock_processor, refund_service, completed_transaction
    ):
        """Refund during processor outage is queued for retry."""
        mock_processor.side_effect = ConnectionError("Processor unavailable")
    ...
    def test_currency_conversion_on_international_refund(
        self, refund_service, international_transaction
    ):
        """International refunds handle currency conversion correctly."""
        result = refund_service.process_refund(
            transaction_id=international_transaction.id,
            amount=international_transaction.amount,
        )

        assert result.status == "completed"
        assert result.currency == international_transaction.original_currency

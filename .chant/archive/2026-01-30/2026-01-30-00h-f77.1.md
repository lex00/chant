...
commits:
- 9fe2883
completed_at: 2026-01-31T15:12:39Z
model: sonnet
---
...
- [x] `WatchConfig` struct with `poll_interval_ms`, `failure` nested config
- [x] `FailureConfig` struct with `max_retries`, `retry_delay_ms`, `backoff_multiplier`, `retryable_patterns`, `on_permanent_failure`
- [x] Parse from `config.md` with defaults (5000ms poll, 3 retries, 60s delay, 2.0 backoff)
- [x] `on_permanent_failure` enum: `skip` or `stop`
- [x] Config validation: reject negative intervals, invalid multipliers

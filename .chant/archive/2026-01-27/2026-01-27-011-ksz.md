...
commits:
- 7820e1e
completed_at: 2026-01-27T23:59:22Z
model: opus
---
...
- [x] Add `all_completed` field to Merge command struct
- [x] Add doc comment explaining the flag
- [x] Update pattern match to extract `all_completed`
- [x] Pass `all_completed` to `cmd_merge()` (remove hardcoded `false`)
- [x] Verify `chant merge --help` shows `--all-completed` flag
- [x] Test: `chant merge --all-completed` runs successfully
- [x] Test: Flag works with `--yes`, `--delete-branch`, etc.

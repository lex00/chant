...
commits:
- 25cce3e
completed_at: 2026-01-29T00:52:41Z
model: opus
---
...
2026-01-29T00:52:41Z

```
I'll start by reading the spec file and understanding the current codebase structure for configuration handling.
I now have a comprehensive understanding of the current config system. Let me also check the .gitignore template and init command to understand the current behavior.
Let me also check the init command to understand how .gitignore is set up and look at the current config.md.
Now let me understand the context better by checking other common patterns used by similar tools (.env/.env.local) and also look at the global config path logic more closely.
Now I have a comprehensive understanding of the codebase. Let me create a todo list and write my design document with recommendations.
Now let me write up the design document by first reading the spec to understand the acceptance criteria format expected, then I'll update the spec with my findings.
Now I'll write my design document with the analysis, recommendation, and implementation plan.
Now I need to update the spec's frontmatter to mark it as completed and add the model.
Now let me verify the changes and commit.
The spec has been committed and the git status shows no uncommitted changes for the spec file. The modified archive files are unrelated to this spec.
I have completed the spec. Here's a summary of what was done:

## Summary

I investigated and documented a design for separating project config from user config in chant.

### Key Findings

1. **Current architecture already has a two-tier system** (`~/.config/chant/config.md` global + `.chant/config.md` project), but `load_merged()` is rarely used - most code uses `load()` which ignores the global config.

2. **Recommended approach**: Hybrid of Options 1 and 2:
   - Use `~/.config/chant/config.md` for user-specific agent configs (primary)
   - Keep `.chant/config.md` for project settings (committed)
   - Add optional `.chant/agents.md` for project-specific agent overrides (gitignored)

3. **Migration path**: Backward compatible - continue reading agents from current locations while showing deprecation warnings and providing a `chant config migrate-agents` command.

4. **Implementation plan**: 4 phased specs covering:
   - Phase 1: Core changes (agents.md support + migration command)
   - Phase 2: Documentation updates
   - Phase 3: Init wizard updates

### All Questions Answered

1. **config.md in .gitignore?** No - project config should be committed, but agents.md should be gitignored
2. **Where should agents live?** Global config primarily, .chant/agents.md for project overrides
3. **Migration?** Graceful with backward compatibility and migration command
4. **Least disruptive?** Phased rollout with deprecation warnings
5. **Secret detection?** Yes, with heuristic pattern matching
6. **Config inheritance?** Yes, with clear merge priority```

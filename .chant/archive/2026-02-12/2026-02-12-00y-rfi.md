...
commits:
- 6ec9589
completed_at: 2026-02-13T04:25:27Z
model: sonnet
---
...
- Parallel mode multiplies this — N workers = N cold starts
...
2026-02-13T04:25:27Z

```
I'll investigate Claude session reuse to reduce agent cold-start time.
## Summary

Session reuse is **not viable** for reducing agent cold-start time.

**Key findings:**
- Claude CLI `--print` mode does not persist sessions to disk
- Measured cold-start: ~2.9s (empty dir) to ~3.8s (chant project)
- Session resume would only save ~1-2s, but requires interactive mode
- Worktree isolation fundamentally incompatible with session state

**Alternative optimization paths documented:**
- Agent pool (pre-started processes)
- Shared MCP server instances
- Lazy CLAUDE.md loading

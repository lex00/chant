...
- [ ] `run_watch(config, flags)` starts event loop with configurable poll interval
- [ ] Each iteration: query in-progress specs, check state, call lifecycle ops
- [ ] `--dry-run` flag logs actions without executing
- [ ] `--once` flag runs single pass then exits
- [ ] SIGINT/SIGTERM handler sets shutdown flag, breaks loop gracefully

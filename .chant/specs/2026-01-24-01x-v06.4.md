...
- [ ] Check `--branch` flag in parallel execution
  - [ ] If `--branch "prefix"` is specified, use that prefix for all worktree branches
  - [ ] If not specified, use default direct-commit mode (merge after completion)
- [ ] Read `config.defaults.branch_prefix` for default prefix in branch mode
- [ ] Verify behavior difference:
  - [ ] **Direct mode (no --branch):** Merge branch to main immediately after completion, delete branch, cleanup worktree
  - [ ] **Branch mode (--branch specified):** Leave branch for `chant reconcile`, cleanup worktree
- [ ] Update summary output to indicate mode used
  - [ ] Show which branch strategy was applied
  - [ ] For branch mode, list branches created for manual review

...
- [ ] Add `--branch` flag to the `work` command
- [ ] When `--branch` is passed, create a branch named `chant/{spec-id}` before invoking the agent
- [ ] The branch should be created from the current HEAD
- [ ] If branch already exists, switch to it instead of failing
- [ ] Update the spec frontmatter with `branch: chant/{spec-id}` after creating/switching

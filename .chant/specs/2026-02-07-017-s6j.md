...
- [ ] Zero direct calls to `spec.force_status()` remain in non-test code (grep verification)
- [ ] All error-recovery paths use `TransitionBuilder::new(spec).force().to(status)`
- [ ] `Spec::force_status` is removed or made `pub(crate)` (not public API)
- [ ] `cargo test` passes

...
**Note**: There are 2 pre-existing test failures (`test_force_flag_bypasses_dependency_check` and `test_force_flag_shows_skipped_dependencies`) caused by a worktree cleanup bug unrelated to this migration. These should remain failing — do not try to fix them.

## Acceptance Criteria

- [ ] `integration/dependency_test.rs` uses TestHarness (no `common::` references)
- [ ] No hand-written spec YAML strings (use SpecFactory)
- [ ] No manual `/tmp` paths
- [ ] All dependency_test tests pass (except the 2 pre-existing failures)

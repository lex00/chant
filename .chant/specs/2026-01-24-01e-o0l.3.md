...
- [ ] finalize_spec() is called after successful agent completion
- [ ] finalize_spec() is called after successful acceptance criteria validation
- [ ] finalize_spec() errors prevent the function from returning success
- [ ] PR creation happens AFTER finalization (so PR URL can be saved to spec)
- [ ] Agent output is appended to spec AFTER finalization (so finalized spec is the base)
- [ ] Test verifies spec is finalized even if PR creation is skipped
- [ ] Test verifies spec is finalized even if agent output appending fails (or not needed)

...
- [ ] `src/operations/cancel.rs` exists with `cancel_spec()` used by both MCP and CLI
- [ ] `src/operations/archive.rs` exists with `archive_spec()` used by both MCP and CLI
- [ ] MCP archive produces same result as CLI archive (date subdirectories, git staging)
- [ ] No inline cancel/archive logic remains in MCP handlers
- [ ] `cargo test` passes

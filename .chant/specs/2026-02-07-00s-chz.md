...
- [x] `integration/config_test.rs` uses TestHarness (no `common::` references)
- [x] No hand-written spec YAML strings (use SpecFactory)
- [x] No manual `/tmp` paths
- [x] All config_test tests pass
- [x] `cargo clippy` passes on the test file

## Output

Migrated config_test.rs to TestHarness/SpecFactory. Added enterprise config helpers. All config tests pass.

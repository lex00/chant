...
- [ ] `TestHarness` struct exists in `tests/support/harness.rs`
- [ ] `SpecFactory` exists in `tests/support/factory.rs`
- [ ] No test uses hardcoded `/tmp` paths
- [ ] `#[serial]` count reduced by at least 50% (from 81 to under 40)
- [ ] `test_integration_*.rs` wrapper files removed (tests run from `integration/` directly)
- [ ] At least 5 existing test files migrated to use `TestHarness`

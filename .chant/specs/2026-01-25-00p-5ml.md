...
- [ ] `src/provider.rs` created with `ModelProvider` trait
- [ ] `ClaudeCliProvider` implements existing behavior
- [ ] `OllamaProvider` calls OpenAI-compatible API with streaming
- [ ] `OpenAiProvider` supports API key authentication
- [ ] Config schema updated with `providers` section
- [ ] `defaults.provider` field added to config
- [ ] Factory function selects correct provider
- [ ] Backward compatible: old configs work unchanged
- [ ] Streaming output works for all providers
- [ ] Clear error message when Ollama not running
- [ ] Clear error message when model not pulled
- [ ] Clear error message for auth failures

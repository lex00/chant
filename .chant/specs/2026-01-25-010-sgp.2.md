...
- [ ] `src/cmd/model.rs` created with the following functions:
...
- [ ] All functions moved with their implementations (~100 lines)
- [ ] Functions remain public and callable from `work.rs`
- [ ] Model detection priority is preserved: CHANT_MODEL → ANTHROPIC_MODEL → config.defaults.model → claude --version parsing
- [ ] `src/cmd/work.rs` updated to import: `use crate::cmd::model::{get_model_name, get_model_name_with_default};`
- [ ] `src/cmd/mod.rs` updated to include `pub mod model;`
- [ ] All usages in `work.rs` and `parallel` execution work correctly
- [ ] `just check` passes

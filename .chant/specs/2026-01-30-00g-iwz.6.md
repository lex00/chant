...
- [ ] `--brief` flag outputs single-line summary
- [ ] `--json` flag outputs JSON
- [ ] `--watch` flag enables live refresh (works with --brief and --json)
- [ ] No flags defaults to regular multi-section output
- [ ] `--brief` and `--json` are mutually exclusive (error if both)

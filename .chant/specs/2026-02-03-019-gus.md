...
- [ ] Add `silent` subcommand to CLI
- [ ] Implement --global flag for global config
- [ ] Implement --off flag to disable
- [ ] Implement --status flag to show current state
- [ ] Auto-manage `.git/info/exclude` entries
- [ ] Auto-remove from `.gitignore` when enabling
- [ ] Add `silent` field to config parsing
- [ ] Suppress git warnings throughout codebase when silent mode active
- [ ] Update docs with silent mode usage
...
- [ ] Consolidate quiet/silent into single `silent` mode
- [ ] Deprecate --quiet flag (keep as hidden alias)
- [ ] Remove CHANT_QUIET env var (or make it set silent mode)
- [ ] Silent mode controls both git behavior AND output verbosity
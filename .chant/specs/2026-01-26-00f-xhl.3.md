...
- [x] Resets spec status to `in_progress` before execution
- [x] Executes spec via existing work flow (reuse work command logic)
- [x] On success, updates status back to `completed`
- [x] On failure, keeps status as `in_progress` or sets to `failed`
- [x] Passes through `--branch` and `--pr` options to work flow

...
- [ ] Add `load_with_branch_resolution` to src/spec.rs
- [ ] Add `branch_exists` and `read_spec_from_branch` helpers
- [ ] Update `cmd_list` to use branch resolution for in_progress specs
- [ ] Show indicator when displaying branch state (e.g., `[branch]` suffix)
- [ ] Update `cmd_show` to use branch resolution
- [ ] Show header indicating branch source
- [ ] Update MCP `chant_spec_get` to use resolution
- [ ] Update MCP `chant_spec_list` to use resolution
- [ ] Add `--main-only` flag to skip resolution (for debugging)
- [ ] Graceful fallback if branch read fails

...
status: in_progress
---
# Fix agent writing invalid model value in spec frontmatter (claude-sonnet-4 → sonnet)
...
- [ ] Spec with `model: claude-sonnet-4` parses successfully
- [ ] `spec.frontmatter.model` returns normalized value `sonnet`
- [ ] Same normalization for opus, haiku variants

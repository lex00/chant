...
- [ ] `chant verify 001` loads spec with ID "001" and validates it has status completed
- [ ] `chant verify --all` loads all specs with status completed
- [ ] `chant verify --label foo` loads all completed specs with label "foo"
- [ ] Error displayed if specified spec ID doesn't exist or is not completed
- [ ] `just test` includes test cases for filtering logic

...
- [x] `operations/update.rs` uses `TransitionBuilder` with `force` opt-in via `UpdateOptions`
- [x] `spec/lifecycle.rs` uses `set_status()` for blocked/unblocked transitions
- [x] `spec_group.rs` uses `TransitionBuilder` for driver status changes
- [x] `cmd/lifecycle/merge.rs --finalize` calls `operations::finalize_spec`
- [x] `cargo test` passes

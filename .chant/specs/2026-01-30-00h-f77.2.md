...
- [ ] `is_completed(spec_id) -> Result<bool>` checks all criteria `[x]` and worktree clean
- [ ] `is_failed(spec_id) -> Result<bool>` checks agent exited, status `in_progress`, criteria incomplete
- [ ] Worktree check uses `git status --porcelain` for uncommitted changes
- [ ] Criteria parsing handles malformed markdown gracefully
- [ ] Returns error if spec file unreadable or worktree inaccessible

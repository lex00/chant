...
- [ ] `src/operations/` module exists with create, finalize, reset, update operations
- [ ] MCP `chant_add` produces identical specs to CLI `chant add` (including derived fields)
- [ ] MCP `chant_finalize` performs the same validation as CLI `chant finalize`
- [ ] MCP `chant_reset` supports the same capabilities as CLI `chant reset`
- [ ] CLI commands delegate to `operations::*` rather than containing inline business logic
- [ ] MCP handlers delegate to `operations::*` rather than containing inline business logic

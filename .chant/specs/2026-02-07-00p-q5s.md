...
- [ ] `test_takeover.rs` uses TestHarness (no `common.rs`, no manual `/tmp` paths)
- [ ] `integration/worktree_test.rs` uses TestHarness consistently across all tests
- [ ] `integration/watch_recovery_test.rs` uses TestHarness (no `common.rs`)
- [ ] Duplicated git helpers (`branch_exists`, `get_branches`) moved to TestHarness or shared module
- [ ] No hand-written YAML strings in any of the 3 files
